...
@Component({
  selector: 'app-my-regional-beneficiary-without-group',
  standalone: false,
  templateUrl: './my-regional-beneficiary-without-group.component.html',
  styleUrl: './my-regional-beneficiary-without-group.component.scss'
})
export class MyRegionalBeneficiaryWithoutGroupComponent implements OnInit, AfterViewInit {
...
@ViewChild(MatSort) sort!: MatSort;
  ...
  */
  ...
  async getAll() {
  this.loading = true;
  await this.surveyService.getAllWithoutGroupByDivipola(0,10).subscribe({
      next: (response: any) => {
  ...
  massive() {
    this.router.navigateByUrl(`/app/my-regional-beneficiary-massive-group`);
  }

  massiveUpdate() {
    this.router.navigateByUrl(`/app/regional-link-update-massive-group`);
  }
