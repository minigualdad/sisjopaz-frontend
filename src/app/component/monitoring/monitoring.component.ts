...
export class MonitoringComponent {
  @Input() reset: boolean = false;
    ...
    async ngOnInit() {
        this.questionGroupService.getAllMonitoreo()
        ...
        this.qty = await this.indexedDbService.getQuantityMonitoring();
        this.detectConnection();
    }

    async onSubmit() {
        await this.indexedDbService.addMonitoring(this.questionData);
        this.uploadToServer(this.questionData)
        this.syncWithServer();
      }
    ...
    async syncWithServer(): Promise<void> {
        const unsyncedRecords = await this.indexedDbService.getAllMonitoring();
        ...
        for (const record of recordsToSync) {
            try {
                ...
                if (response.ok) {
                    ...
                    await this.indexedDbService.deleteMonitoring(record.id);
                    success = true;
                }
            } catch (error) {
    ...
    private async uploadToServer(record: any): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            if (this.completedAllQuestions) {
                ...
                this.monitoringService.create(questionJson)
                    .subscribe({
                        next: (response: any) => {
                        ...
                        error: (error: any) => {
                            console.error(error);
                            Swal.fire('Operación incorrecta', 'No se ha podido crear el monitoreo', 'error');
                        }
                    });
            }
        });
    ...
    showSurvey() {
        this.surveyService.show(this.survey.id)
            .subscribe((response: any) => {
                ...
                this.professionalTeamId = this.survey.professionalTeamId;
            })
    }
